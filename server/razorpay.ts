...
export function getNextBillingDate(subscription: any): Date {
  return new Date(subscription.current_end * 1000);
}

// Calculate proration for plan changes
export async function calculateProration(currentPlanId: string, newPlanId: string, subscription: any): Promise<number> {
  try {
    const currentPlan = await razorpay.plans.fetch(currentPlanId);
    const newPlan = await razorpay.plans.fetch(newPlanId);
    
    const currentAmount = currentPlan.item.amount;
    const newAmount = newPlan.item.amount;
    
    // Calculate remaining days in current period
    const now = Date.now() / 1000;
    const remainingDays = (subscription.current_end - now) / (24 * 60 * 60);
    const totalDays = (subscription.current_end - subscription.current_start) / (24 * 60 * 60);
    
    // Calculate proration
    const unusedCredit = (currentAmount * remainingDays) / totalDays;
    const prorationAmount = newAmount - unusedCredit;
